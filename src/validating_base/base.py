...
from validating_base.decorators_internal import validate_init, validated_decorated
...
def update_validated_methods(cls: C) -> C:
    """Update the `__validated_methods__` attribute to include all the validated methods on the class."""
    ...
    for scls in cls.__bases__:  # type: ignore[attr-defined]
        for name in getattr(scls, "__validated_methods__", ()):
            validated_methods.add(name)

    # Also add any other newly added validated methods.
    for name, value in cls.__dict__.items():
        if getattr(value, "__isruntimevalidated__", False):
            validated_methods.add(name)

    cls.__validated_methods__ = frozenset(validated_methods)  # type: ignore[attr-defined]
    return cls
...
class ValidatingBaseClass(metaclass=ValidatingBaseClassMeta):
    ...
    def _validate_self(self) -> None:
        ...
        for validated_method_name in self.__validated_methods__:
    ...
    def __getattribute__(self, __name: str) -> Any:
        ...
        if __name in ["_self_validated", "__validated_methods__"]:
            return method

        if callable(method) and self._self_validated and __name in self.__validated_methods__:
